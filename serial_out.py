...
# UART TX on GP0 -> connect to Pi RXD (GPIO15)
...
MODIFIERS = {'shift_l', 'shift_r', 'ctrl', 'alt'}
    ...
    if len(keys) == 1:
        ...
        if key in MODIFIERS:
            ...
            if key in active_modifiers:
                ...
                oneshot_modifiers.discard(key)
                send('M-:' + key)
            else:
                kbd.press(kc)
                active_modifiers.add(key)
                oneshot_modifiers.add(key)
                send('M+:' + key)
        elif key in HID_MAP:
            kc = HID_MAP[key]
            kbd.press(kc)
            ...
            if oneshot_modifiers:
                for mod in list(oneshot_modifiers):
                    kbd.release(MODIFIER_HID[mod])
                    send('M-:' + mod)
                    active_modifiers.discard(mod)
                oneshot_modifiers.clear()
    else:
        mods = [k for k in keys if k in MODIFIERS]
        regs = [k for k in keys if k not in MODIFIERS and k in HID_MAP]

        for m in mods:
            kbd.press(MODIFIER_HID[m])
            send('M+:' + m)
        for r in regs:
            kbd.press(HID_MAP[r])
            send('P:' + r)
